...
public class MaterialAccount {
	...
	public MaterialAccount(Resources resources, String title, String subTitle,
			int photo, Bitmap background) {
		this.title = title;
	...
	public MaterialAccount(Resources resources, String title, String subTitle,
			String bio, String website, int photo, int background) {
		this.title = title;
		this.subTitle = subTitle;
		this.resources = resources;
		this.bio = bio;
	...
	public MaterialAccount(Resources resources, String title, String subTitle,
			Bitmap photo, Bitmap background) {
		this.title = title;
		this.subTitle = subTitle;
		this.resources = resources;
	...
	public View getSectionView(Context ctx, Typeface font,
			MaterialSectionListener listener, boolean rippleSupport,
			int position) {
		if (sectionView == null) {
			sectionView = new MaterialSection(ctx, MaterialSection.ICON_40DP,
					rippleSupport, MaterialSection.TARGET_LISTENER);
	...
	private class ResizePhotoResource extends
		...
		protected BitmapDrawable doInBackground(Integer... params) {
			...
			Bitmap photo = Utils.resizeBitmapFromResource(resources, params[0],
					photoSize.x, photoSize.y);

			circularPhoto = new BitmapDrawable(resources,
					Utils.getCroppedBitmapDrawable(photo));
	...
	private class ResizePhotoBitmap extends
			AsyncTask<Bitmap, Void, BitmapDrawable> {

		@Override
		protected BitmapDrawable doInBackground(Bitmap... params) {
			Point photoSize = Utils.getUserPhotoSize(resources);

			Bitmap photo = Utils.resizeBitmap(params[0], photoSize.x,
					photoSize.y);
			params[0].recycle();
	...
	private class ResizeBackgroundResource extends
			AsyncTask<Integer, Void, BitmapDrawable> {
		@Override
		protected BitmapDrawable doInBackground(Integer... params) {
			Point backSize = Utils.getBackgroundSize(resources);

			Bitmap back = Utils.resizeBitmapFromResource(resources, params[0],
	...
	private class ResizeBackgroundBitmap extends
			AsyncTask<Bitmap, Void, BitmapDrawable> {
